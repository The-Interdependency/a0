...
Branch base: `feat/replit-backend-foundation@62d01ade893db5152907df0b5c5173f4f77d43f0`
...
At the branch base, schema ownership is split among:
...
3. executable SQL and command surfaces — startup, request-path, and post-merge mutation in:
...
- contract checks that require the known mutation sites and prove the capture/status boundaries.
...
| `migrations/script.py.mako` | create | bounded future revision template | generated revision missing local declarations | template review before first revision |
...
| `scripts/sche_capt_live_v0.0.0alpha.py` | create | capture schema-only PostgreSQL evidence | credential leakage or volatile output | mocked command/redaction and deterministic-normalization checks |
...
The next operation is read-only against the Replit database:
...
2. record a database backup/snapshot identifier;
3. record PostgreSQL server and `pg_dump` major versions;
4. verify the application branch/commit;
5. stop if the database is actively undergoing an unrelated migration.

The capture tool uses `pg_dump --schema-only --no-owner --no-privileges --no-comments --quote-all-identifiers`. It writes SQL and an adjacent SHA-256 file. It does not execute SQL.
...
- remove schema mutation from `python/main.py`, `server/auth/setup.ts`, and `python/routes/billing.py`;
...
- Replit Agent is currently occupied, so the clean PR #93 gate and live schema capture have not executed.
- Exact live schema digest and PostgreSQL/`pg_dump` versions remain pending capture.
