...
## Identity
**a0p is a research instrument, not a product.** It is the public-facing instance name for `a0` (this codebase / repository), currently governed by the release gate in `PUBLIC_RELEASE.md`. It explores agent / energy-provider / PCNA dynamics in the open. Public use is free; code-altering access is restricted to the owner (Erin) and a small set of explicitly invited collaborators. The instrument is funded by donations and does not solicit subscribers.
...
- **Console-tab regression guards (Task #86):**
  ...
  - **CI integration (Task #92):** the `check-console-tabs` job in `.github/workflows/deploy.yml` boots an ephemeral Postgres + uvicorn backend on every push to `main` and runs the script. The `deploy` job declares `needs: check-console-tabs`, so the Cloud Run deploy is blocked when the script exits non-zero, which happens for either (a) a tab returned by the API with no custom renderer and no sections, or (b) an orphan entry in `CUSTOM_TAB_RENDERERS` whose `tab_id` is no longer returned by `/api/v1/ui/structure`. See `DEPLOYMENT.md` → "Pre-deploy checks".
